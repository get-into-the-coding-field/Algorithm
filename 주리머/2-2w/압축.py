import string 
ascii = {i: idx + 1 for idx, i in enumerate(list(string.ascii_uppercase))}
def solution(msg):
    chr = ""
    result = []
    nextValue = 27
    ...
    for i in range(len(msg)):
        ...
        if chr not in ascii:
            ascii[chr] = nextValue
            nextValue += 1
            result.append(ascii[chr[:-1]])
            chr = chr[-1]

    if chr != "":
        result.append(ascii[chr])
            
    return result

'''
존재하면 chr에 추가하고 계속
없으면 등록하고 [-1]만큼을 등록하고 chr에 뻄
...
같으면 nextIdx += 1
다르다면 currentIdx += 1
arr[currentIdx:nextIdx+1]를 사전에 등록
압축
